...
interface Exam {
  ...
  hasSubmitted?: boolean; // New flag for single attempt check
  [key: string]: any; 
}
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        const q = query(examsRef, where('classId', 'in', joinedClassIds), where('status', '==', 'ONGOING'));
        ...
        // Check each exam against the responses collection
        const examsWithStatus = await Promise.all(fetchedExams.map(async (exam) => {
            ...
            return { ...exam, hasSubmitted: !respSnap.empty };
        }));

        examsWithStatus.sort((a, b) => (a.examDate?.seconds || 0) - (b.examDate?.seconds || 0));
            ...
            <View key={exam.id} style={styles.card}>
              ...
              <View style={styles.detailsRow}>
                ...
                <View style={styles.detailItem}>
                  ...
                  <Text style={styles.detailText}>{exam.durationMinutes || 60} mins</Text>
                </View>
              </View>

              {exam.hasSubmitted ? (
                <View style={[styles.startButton, { backgroundColor: '#10B981', opacity: 0.8 }]}>
                    <Ionicons name="checkmark-circle" size={18} color="#FFF" style={{ marginRight: 8 }} />
                    <Text style={styles.startButtonText}>Already Submitted</Text>
                </View>
              ) : (
                <TouchableOpacity 
                    style={styles.startButton}
                    activeOpacity={0.8}
                    onPress={() => router.push({ pathname: '/student/(exams)/takeExam', params: { examId: exam.id } })}
                >
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#FFF', borderRadius: 20, padding: 20, marginBottom: 16, elevation: 3, borderWidth: 1, borderColor: '#F3F4F6' },
  ...
  startButton: { marginTop: 20, backgroundColor: '#4F46E5', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 14, borderRadius: 12 },
  startButtonText: { color: '#FFF', fontSize: 15, fontWeight: '700', marginRight: 6 }
});