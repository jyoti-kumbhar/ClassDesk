  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        `You have been registered as a ${role}. A verification email has been sent.`,
        [
          { 
            text: "Go to Login", 
      ...
      <Modal visible={showRolePicker} transparent animationType="fade" onRequestClose={() => setShowRolePicker(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowRolePicker(false)}>
          <View style={styles.modalContent}>
            ...
            <FlatList data={roles} keyExtractor={(item) => item} renderItem={renderRoleItem} />
          </View>
        </TouchableOpacity>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  inputWrapper: { flexDirection: "row", alignItems: "center", backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#E2E8F0", borderRadius: 12, paddingHorizontal: 15, height: 55 },
  inputIcon: { marginRight: 10 },
  textInput: { flex: 1, fontSize: 16, color: "#2D3748" },
  inputText: { flex: 1, fontSize: 16, color: "#2D3748" },
  signupButton: { backgroundColor: "#4461F2", borderRadius: 12, height: 55, justifyContent: "center", alignItems: "center", marginTop: 30, marginBottom: 20, elevation: 4 },
  ...
  linkText: { color: "#4461F2", fontWeight: "bold", fontSize: 14 },
  ...
  selectedModalItemText: { color: "#4461F2", fontWeight: "bold" },
});